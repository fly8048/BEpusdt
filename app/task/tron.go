...
var usdtTrc20ContractAddress = []byte{0x41, 0xec, 0xa9, 0xbc, 0x82, 0x8a, 0x30, 0x05, 0xb9, 0xa3, 0xb9, 0x09, 0xf2, 0xcc, 0x5c, 0x2a, 0x54, 0x79, 0x4d, 0xe0, 0x5f}
	...
	for _, trans := range bok.GetTransactions() {
		...
		for _, contract := range itm.GetRawData().GetContract() {
			...
			if contract.GetType() == core.Transaction_Contract_TriggerSmartContract {
				...
				str := string(t.base58CheckEncode(foo.GetContractAddress()))
				fmt.Println("Contract Address:", str)
				str2 := string(t.base58CheckEncode(usdtTrc20ContractAddress))
				fmt.Println("Contract Address2:", str2)
